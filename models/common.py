...
class CABlock(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):  # Only one input (x) and optional mask
        shortcut = q 
        q = self.norm1(q)
        ...
        if mask is not None:
            x = x * mask.unsqueeze(-1)  # Apply mask to the attention output

        x = shortcut + x  # Residual connection

        out = x + self.mlp(self.norm2(x))  # Feedforward network
